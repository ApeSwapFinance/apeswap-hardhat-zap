...
contract ApeSwapZap is IApeSwapZap, ReentrancyGuard {
    ...
    event Zap(address inputToken, uint256 inputAmount, address[] underlyingTokens);
    event ZapNative(uint256 inputAmount, address[] underlyingTokens);
    ...
    function _zapInternal(
        ...
        emit Zap(address(inputToken), inputAmount, underlyingTokens);
    }
    ...
    function _zapNativeInternal(
        ...
        emit ZapNative(inputAmount, underlyingTokens);
    }
    ...
    function _zapPrivate(
        ...
        if (underlyingTokens[0] == WNATIVE) {
            // Ensure WNATIVE is called last
            _transfer(underlyingTokens[1], vars.amount1 - amountB, native);
            _transfer(underlyingTokens[0], vars.amount0 - amountA, native);
        } else {
            _transfer(underlyingTokens[0], vars.amount0 - amountA, native);
            _transfer(underlyingTokens[1], vars.amount1 - amountB, native);
        }
